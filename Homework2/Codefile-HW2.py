...
background_image_filename = 'c:/Users/vsamu/Downloads/curve_pattern.png'
...
def drawPolylines(color='GREEN', thick=3):
    ...
    for i in range(count-1):
        ...
        b=(pts[i][1])-(m*pts[i][0])
        x_main = np.linspace(pts[i][0],pts[i+1][0],100)
        y_main=m*x_main+b
    for i in range(len(x_main)):
        drawPoint((x_main[i],y_main[i]),color)
        #drawLine(x_main, y_main, color)
